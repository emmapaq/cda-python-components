...
class TemperatureEmulatorTaskTest(unittest.TestCase):
    ...
    Bash on Ubuntu on Windows environment and then execute
    this test case from the command line, as it will likely
    fail if run within an IDE in native Windows.
    ...
    def testReadEmulator(self):
        ...
        if sd1:
            ...
            # Use %s instead of %f to safely log None values
            logging.info("SensorData: %s - %s", sd1.getValue(), str(sd1))
        ...
        if sd2:
            ...
            logging.info("SensorData: %s - %s", sd2.getValue(), str(sd2))
